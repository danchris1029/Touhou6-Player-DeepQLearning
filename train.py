#   Christian G.
#   Description:
#   Project to experiement with a Q-Learning model to see if I could make a bot
#   that can play Touhou 6: Embodiment of Scarlet Devil.
#   Took about 12 hours to put together and experiement to get some decent
#   output.

#   Note: Bot couldn't make it past the first level, maybe nudging some
#   variables could ecourage better performance.
import ctypes
import win32ui
import win32process
import win32api
...
def take_screenshot():
    ...
    img = img.resize((25, 25))                              # Edit dimensions if not enough memory to
                                                            # allocate
    img = np.asarray(img)
    img = img[:, :, 0]                                      # Convert RBG image to greyscale for less
                                                            # memory usage but still visible
    return np.ndarray.flatten(img)

def takeAction(action):                                  # Ai control is slow, could be implemented
                                                         # better to allow more efficient flexibility
...
for i in range(n_games):                
    ...
    while not done:
        ...
        if (t - prev_t > 3):                                    #TODO: Messy reward system is messy, should
                                                                #be cleaned up.
            prev_t = t                      
            reward_multiplier += 1
